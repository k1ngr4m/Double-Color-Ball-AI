...
class LotteryDataFetcher:
        ...
        try:
            ...
            for row in rows:
                ...
                try:
                    ...
                    # 提取蓝球
                    blue_ball = [cols[i].text.strip() for i in range(6, 8)]
    ...
    def save_to_json(self, data, filename="sports_lottery_data.json", preserve_history=True):
        """
        保存数据到 JSON 文件
        ...
        try:
            if preserve_history:
                ...
                # 同时更新到 ../data/sports_lottery_data.json
                try:
                    web_data_path = os.path.join(os.path.dirname(filename), '..', 'data', 'sports_lottery_data.json')
